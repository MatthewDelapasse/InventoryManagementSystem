...
namespace LeasedTicketScreen
{
    public partial class frmLeasedTicket : Form
    {
        ...
        SqlConnection deviceConnection;
        ...
        private void frmLeasedTicket_Load(object sender, EventArgs e)
        {
            ...
            cboDeviceTag.DataBindings.Add("Text", leasedTicketTable, "DeviceTag");
            txtOtherItems.DataBindings.Add("Text", leasedTicketTable, "DeviceName");
            txtDateCreated.DataBindings.Add("Text", leasedTicketTable, "SerialNumber");
        ...
        private bool ValidateData()
        {
            ...
            if (cboDeviceTag.Text.Trim().Equals(""))
            {
        ...
        private void FillDevices(string connection)
        {
            string strConnection = "SELECT DeviceTag FROM Inventory ORDER BY DeviceTag";

            using (deviceConnection = new SqlConnection(connection))
            {
                deviceCommand = new SqlCommand(strConnection, deviceConnection);
                deviceReader = deviceCommand.ExecuteReader();

                while (deviceReader.Read())
                {
                    deviceAdapter = new SqlDataAdapter();
                    deviceAdapter.SelectCommand = deviceCommand;
                    deviceTable = new DataTable();
                    deviceAdapter.Fill(deviceTable);
                    deviceView = new DataView(deviceTable);

                    for (int d = 0; d < deviceTable.Rows.Count; d++)
                    {
                        cboDeviceTag.Items.Add(deviceTable.Rows[d]["DeviceTag"]);
                    }
                }
